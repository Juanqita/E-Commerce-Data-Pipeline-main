...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    data = pd.merge(items_orders, products, on="product_id")
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    order_purchase_amount_per_date = filtered_dates.groupby(filtered_dates["order_purchase_timestamp"].dt.date).agg(order_count=("order_id", "count")).reset_index().rename(columns={"order_purchase_timestamp": "date"})
