...
def plot_revenue_per_state(df: DataFrame):
    ...
    """
    fig = px.treemap(
        df, path=["state"], values="revenue_per_state", width=800, height=400
    )
...
def plot_freight_value_weight_relationship(df: DataFrame):
    ...
    ax.set_xlabel("product_weight_g")
...
def plot_order_amount_per_day_with_holidays(df: DataFrame):
    ...
    ax.plot(df['date'], df['order_count'], color="green", linestyle='-', label='Orders')

    for holiday in df.loc[df['holiday'], 'date']:
        ax.axvline(holiday, color="blue", linestyle=':', alpha=0.7, label='Holiday')
    
    handless, labels = ax.get_legend_handles_labels()
    by_label = dict(zip(labels, handless))
    ax.legend(by_label.values(), by_label.keys())

    ax.set_title("Order Amount per Day with Holidays Highlighted")
    ax.set_xlabel("Date")
